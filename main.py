...
if st.session_state.page == "Datos de compra y financiación":
    ...
    def chatbot_query(user_input):
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Eres un asistente inmobiliario ayudando al usuario a encontrar la mejor vivienda según sus preferencias. No respondes ninguna otra pregunta."},
                {"role": "user", "content": user_input}
            ]
        )
        return response.choices[0].message.content

    # Property Search Based on Chatbot Output
    def find_best_match(query):
        filtered_df = data[data.apply(lambda row: query.lower() in row.to_string().lower(), axis=1)]
